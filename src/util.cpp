...
const char * const BITCOIN_CONF_FILENAME = "teddycoin.conf";
const char * const BITCOIN_PID_FILENAME = "teddycoin.pid";
...
#else
    const char* pszModule = "teddycoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Teddycoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Teddycoin";
#else
    // Unix
    return pathRet / ".teddycoin";
#endif
#endif
}
